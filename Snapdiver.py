...
if len(spacename)>1:
    ...
    if len(file) > 1:
        ...
        s#t.write('Let\'s get started! Pulling from: ', spacename, ':sunglasses:')
        ...
        proposal_list = Snapshots['proposals_title'].to_list()
        proposal = st.selectbox("Select Proposal", options=list(proposal_list))

        prop_id= db.query("select distinct proposals_title,proposals_id "
                 "from proposal_list  where proposals_title = proposal").df()
